...
void	Command::handleCommand(int socket, vector<string> split, Server& server){
	...
	if (_commands.find(command) != _commands.end()){ // to check if the command exist in the map
		 (server.*_commands[command])(socket, split); //this killed me copy paste in chat gpt and learned it yourself
	} else {
