...
void Server::Topic(int socket, vector<string>& arg, Client cl) {
	...
	cout << "TOPIC TOPIC TOPIC " << topic << endl;
	if (arg.size() == 0){
		cout << "TOPIC current client needmore param\n";
		return _channels[channelName].broadcast(TOPIC(currentClientNickname, currentClientUsername, channelName, topic.c_str()));
	}
	...
	if ((_channels[channelName].getCMode() & t) != t) {
		...
		if (checkChannelName(channelName) == false) {
			...
			_channels[channelName].setTopicNickname(currentClientNickname);
			_channels[channelName].broadcast(TOPIC(currentClientNickname, currentClientUsername, channelName, topic.c_str()));
		}
	}
}
