...
void Server::Topic(int socket, vector<string>& arg, Client cl) {
	...
	if (arg.size() == 0)
		...
		} else if (_channels[channelName].isUserOperator(currentClientNickname) == false) {
			return currentClient.sendMessage(ERR_NOPRIVILEGES(currentClientNickname));
		} else if (_channels[channelName].getTopic() == "") {
			return currentClient.sendMessage(RPL_NOTOPIC(currentClientNickname, channelName));
		} else {
